...
import { evaluationAPI, predictionsAPI, settingsAPI } from '../services/api'
...
export default function Evaluation() {
  ...
  useEffect(() => {
    Promise.all([evaluationAPI.getSummary(), evaluationAPI.getSystemMetrics(), predictionsAPI.getModelInfo(), settingsAPI.getFirebaseStatus()])
      .then(([summaryData, systemData, modelData, firebaseData]) => {
  ...
  const modelVersion = modelInfo?.model_version || metrics.modelVersion || 'rf-v1'
  const trainingSamples = (metrics.n_train_samples || 0) + (metrics.n_test_samples || 0)

  const exportReport = () => {
    const report = { generatedAt: new Date().toISOString(), baseline, modelMetrics: metrics, systemMetrics, firebaseStatus, summary }
    const url = URL.createObjectURL(new Blob([JSON.stringify(report, null, 2)], { type: 'application/json' }))
    const link = document.createElement('a')
    link.href = url
    link.download = 'infrasight-evaluation-report.json'
    link.click()
  ...
  const footer = (doc) => {
    ...
    for (let index = 1; index <= pageCount; index += 1) {
      ...
      doc.text(`InfraSight - Great Zimbabwe University - ${modelVersion}`, 20, 285)
    }
  }
  ...
  const exportPdf = () => {
    ...
    doc.setFontSize(24)
    doc.text('InfraSight Evaluation Report', 20, 45)
    doc.setTextColor(20)
    doc.setFontSize(16)
    doc.text('Great Zimbabwe University', 20, 65)
    doc.setFontSize(13)
    doc.text('ICT Fault Detection and Predictive Maintenance System', 20, 82)
    doc.text(`Date generated: ${now}`, 20, 100)

    doc.addPage()
    doc.setFontSize(18)
    doc.text('Operational KPI Comparison', 20, 25)
    doc.setFontSize(11)
    let y = 45
    doc.text('Metric', 20, y)
    doc.text('Before', 85, y)
    doc.text('After', 120, y)
    doc.text('Improvement', 155, y)
    baseline.forEach(item => {
      y += 12
      doc.text(item.label, 20, y)
      doc.text(`${item.before}${item.unit}`, 85, y)
      doc.text(`${item.after}${item.unit}`, 120, y)
      doc.text(`${Math.abs(improvement(item)).toFixed(1)}%`, 155, y)
    })

    doc.addPage()
    doc.setFontSize(18)
    doc.text('Model Performance', 20, 25)
    doc.setFontSize(12)
    const performance = [
      ['Accuracy', percent(metrics.accuracy)],
      ['Precision', percent(metrics.precision)],
      ['Recall', percent(metrics.recall)],
      ['F1 Score', percent(metrics.f1_score || metrics.f1Score)],
      ['Model Version', modelVersion],
      ['Training Samples', String(trainingSamples)],
    ]
    y = 45
    performance.forEach(([label, value]) => { doc.text(`${label}: ${value}`, 20, y); y += 11 })
    y += 8
    doc.text('Top 3 Feature Importances', 20, y)
    features.slice(0, 3).forEach(feature => { y += 11; doc.text(`${feature.feature}: ${percent(feature.importance)}`, 25, y) })

    doc.addPage()
    doc.setFontSize(18)
    doc.text('System Activity Summary', 20, 25)
    doc.setFontSize(12)
    y = 45
    const systemRows = [
      ['Total devices monitored', systemMetrics?.devicesMonitored],
      ['Telemetry windows built', systemMetrics?.windowsBuilt],
      ['Total predictions made', systemMetrics?.totalPredictions],
      ['Total alerts generated', systemMetrics?.activeAlerts],
      ['Fault prone predictions', systemMetrics?.faultPronePredictions],
      ['Normal predictions', systemMetrics?.normalPredictions],
      ['Firebase mode', firebaseStatus?.datastore_mode || 'unknown'],
    ]
    systemRows.forEach(([label, value]) => { doc.text(`${label}: ${value ?? 0}`, 20, y); y += 12 })
    footer(doc)
    doc.save('infrasight-evaluation-report.pdf')
  }
